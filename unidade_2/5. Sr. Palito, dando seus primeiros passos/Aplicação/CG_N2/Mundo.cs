    ...
    class Mundo : GameWindow
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            obj_Reta = new SegReta("R1",
                                   null,
                                   new Ponto4D(),
                                   Matematica.GerarPtosCirculo(angulo, raio)
                                   );
        ...
        protected override void OnKeyDown(OpenTK.Input.KeyboardKeyEventArgs e)
        {
            if (e.Key == Key.H)
                ...
                objetoSelecionado.PrimitivaTipo = PrimitiveType.Lines;
                        ...
                        || e.Key == Key.Left)
            {
                Ponto4D pto = objetoSelecionado.RetornaPonto(0);
                pto.X--;
                objetoSelecionado.PontosAlterar(pto, 0);
                pto = objetoSelecionado.RetornaPonto(1);
                pto.X--;
                objetoSelecionado.PontosAlterar(pto, 1);
            }
            else if (e.Key == Key.W
                        || e.Key == Key.Right)
            {
                ...
                if (raio > 0)
                {
                    Ponto4D pto = objetoSelecionado.RetornaPonto(0);
                    raio--;
                    Ponto4D ptoNew = Matematica.GerarPtosCirculo(angulo, raio);
                    ptoNew.X += pto.X;
                    ptoNew.Y += pto.X;
                    pto = objetoSelecionado.RetornaPonto(1);
                    pto.X = ptoNew.X;
