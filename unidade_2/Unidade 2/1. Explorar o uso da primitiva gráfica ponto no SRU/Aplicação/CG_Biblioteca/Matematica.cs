...
namespace CG_Biblioteca
{
  ...
  public abstract class Matematica
  {
    ...
    public static Ponto4D GerarPtosCirculo(int xOrigem, int yOrigem, double angulo, double raio)
    {
      Ponto4D pto = new Ponto4D();
      pto.X = ((raio * Math.Cos(Math.PI * angulo / 180.0)) + xOrigem);
      pto.Y = ((raio * Math.Sin(Math.PI * angulo / 180.0)) + yOrigem;
      pto.Z = 0;
      return (pto);
    }
